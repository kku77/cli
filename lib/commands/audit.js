...
class VerifySignatures {
  ...
  async run () {
    ...
    if (!this.auditedPackages.size) {
      throw new Error('found no dependencies to audit that where installed from a registry')
    }
    ...
    const auditedPlural = this.auditedPackages.size > 1 ? 's' : ''
    const timing = `audited ${this.auditedPackages.size} package${auditedPlural} in ` +
    ...
    // TODO log packages that weren't verified but weren't missing/invalid i.e.
    // from registries with no keys
    ...
    } else if (hasNoInvalidOrMissing) {
      this.appendOutput(`found no packages with missing or invalid registry signatures`)
    }
